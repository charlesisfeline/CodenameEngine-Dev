...
function postCreate() {
	...
	addChildAt(background = new Bitmap(Assets.getBitmapData(Paths.image("menus/soundTray/volumebox"))), index);
	_defaultScale = 0.8;
	screenCenter();
}
...
function regenerateBars(event) {
	...
	{
		var bar:Bitmap;
		addChild(bar = new Bitmap(Assets.getBitmapData(Paths.image("menus/soundTray/bar_" + (i + 1)))));
